...
import { LessonSessionMessageRole, prisma } from "@acme/db"

import { env } from "~/env.server"
import { calculateAudioDuration, parseWAVFile } from "./utils/audio"
import { getAction } from "./utils/get-action"
import { getLessonInstructions } from "./utils/get-lesson-instructions"
...
export const handleLessonSession = async ({ lessonSessionId, profile, organization, socket }: Props) => {
  const lessonSession = await prisma.lessonSession.findUnique({
    where: { id: lessonSessionId, profileId: profile.id, organizationId: organization.id },
    include: {
      lesson: true,
    },
  })
  ...
  if (!lessonSession) {
    console.error(`Could not find lesson session, profileId: ${profile.id}, organizationId: ${organization.id}, lessonSessionId: ${lessonSessionId}`)
    socket.disconnect()
    return
  }
  ...
  console.log(`🟢 Lesson session started for profileId: ${profile.id} and lessonSessionId: ${lessonSessionId} at time: ${sessionStartTime}`)
  ...
  let vocabularyLearnedCount = 0 // in units
  ...
  const updateLessonSessionDuration = async () => {
    const currentTime = Date.now()
    const currentDuration = Math.floor((currentTime - sessionStartTime) / 1000) // duration in seconds

    console.log(
      `🕑 Lesson session: ${currentDuration}s total, user spoke: ${totalUserSpeakingDuration.toFixed(1)}s, AI spoke: ${totalAiSpeakingDuration.toFixed(1)}s`,
    )
  ...
  const endLessonSession = async () => {
    ...
    const finalDuration = await updateLessonSessionDuration()
    ...
    await incrementProfileStats({
      ...
      secondsInLessons: finalDuration,
      vocabularyLearned: vocabularyLearnedCount,
      wordsSpoken: wordsSpokenCount,
    })

    console.log(`🔴 Lesson session ended after ${finalDuration}s for profileId: ${profile.id} and lessonSessionId: ${lessonSessionId}`)
  ...
  ws.on("open", function open() {
    ...
    const lessonInstructions = getLessonInstructions({
    ...
    switch (parsedMessage.type) {
      ...
      case "response.output_audio_transcript.done": {
        ...
        await prisma.lessonSessionMessage.create({
          data: {
            sessionId: lessonSessionId,
            role: LessonSessionMessageRole.ASSISTANT,
            content: parsedMessage.transcript,
          },
        })
        ...
        const actionResult = await getAction({
          transcript: parsedMessage.transcript,
          profile: profile,
          lessonSessionMessages: lessonSessionMessages,
        })

        if (actionResult.vocabularyType) {
          await prisma.lessonSessionVocabulary.create({
            data: {
              sessionId: lessonSessionId,
          ...
          vocabularyLearnedCount = vocabularyLearnedCount + 1
        }
