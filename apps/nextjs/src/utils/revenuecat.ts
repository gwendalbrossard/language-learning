...
export const isUnlimited = (profile: RouterOutputs["profile"]["me"]) => {
  if (!profile.revenueCatCustomer) return false

  const customer = profile.revenueCatCustomer as Customer
